...
const cartSlice = createSlice({
    ...
    reducers:{
        ...
        incrQuantity:(state,actionFromCart)=>{
        ...
        decrQuantity:(state,actionFromCart)=>{
            const existingProduct = state.find(item=>item.id==actionFromCart.payload.id)
            existingProduct.quantity--
            existingProduct.totalPrice = existingProduct.quantity * existingProduct.price
            const remainingProducts = state.filter(item=>item.id!=actionFromCart.payload.id)
...
export const {addToCart,removeCartItem,incrQuantity,decrQuantity,emptyCart} = cartSlice.actions
